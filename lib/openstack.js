...
var getMeasurement = function(tokenId, instanceId, measurementType) {
  ...
  var measurementUrl = 'http://' + ceilometerHostname + ':' +
    ceilometerPort + '/v2/meters/' + measurementType +
  ...
  options.transform = function(data) {
    ...
    if (Object.keys(data).length == 0) {
      throw new Error(`No ${measurementType} data received for instance ${instanceId} after ${currentTime}`);
    } else {
      if ('avg' in data[0]) {
        // use duration_end from Ceilometer response as the timestamp
        return {value: data[0].avg, time: data[0].duration_end};
      } else {
  ...
  return rp(options)
    .then(function(data) {
      db.writeMeasurement(measurementType, instanceId, data.value, data.time);
    }, reportError);
};
