...
var proxyMeasurementTypes = [
  {endPoint:"cachediskutilization", description:"Cache disk utilization"},
  {endPoint:"cachememkutilization", description:"Cache memory utilization"},
  {endPoint:"cpuusage", description:"CPU consumed by Squid for the last 5 minutes"},
  {endPoint:"diskhits", description:"Disk hits percentage for the last 5 minutes (hits that are logged as TCP_HIT)"},
  {endPoint:"hits", description:"Cache hits percentage of all requests for the last 5 minutes"},
  {endPoint:"hits_bytes", description:"Cache hits percentage of bytes sent for the last 5 minutes"},
  {endPoint:"httpnum", description:"Number of HTTP requests received"},
  {endPoint:"memoryhits", description:"Memory hits percentage for the last 5 minutes (hits that are logged as TCP_MEM_HIT)"},
  {endPoint:"usernum", description:"Number of users accessing the proxy"}
];

var sbcMeasurementTypes = [
  ...
  {endPoint: 'rtp_pack_out', description: 'Number of outgoing RTP'},
...
var routes = [{
  ...
  config: {
    ...
    handler: function(request, reply) {
      reply(measurementTypes.concat(proxyMeasurementTypes)
           .concat(sbcMeasurementTypes));
    }
  }
}, {
  method: 'POST',
...
.concat(generateMultipleMeasurementRoutes(proxyMeasurementTypes))
.concat(generateMultipleMeasurementRoutes(sbcMeasurementTypes));
